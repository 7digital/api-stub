...
BasketHandler.prototype.getBasket = function getBasket(req, res, next) {
	...
	const twoItemBaskets = [
		'basket-with-two-items',
		'17432a95-ec84-44e6-8d3d-93c6a696b836',
		'prepare-purchase-error'
	];

	if (twoItemBaskets.indexOf(basketId) !== -1) {
...
BasketHandler.prototype.removeItem = function removeItem(req, res, next) {
	eyes.inspect(baskets);
	const basket = getDefaultBasketResponse(req.query.basketId);
	baskets[req.query.basketId] = basket;
	return res.send(basket);
};

BasketHandler.prototype.purchaseStatus = function purchaseStatus(req, res, next) {
	if (req.params.id === ':id') {
		return next(new Error('Basket ID not sent'));
	}

	if (req.params.id === 'purchase-failure') {
		return res.sendfile(path.join(__dirname, '../responses/basket/status/failure.xml'));
	}

	if (req.params.id === 'purchase-success') {
		return res.sendfile(path.join(__dirname, '../responses/basket/status/success.xml'));
	}

	return next(new Error('No response defined'));
};
