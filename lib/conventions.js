...
function findMatchingParameter(matcher, req) {
	...
	for (param in paramsToSearch) {
		if (!Object.prototype.hasOwnProperty.call(paramsToSearch, param))
			continue;
...
ConventionalHandler.prototype.serveDefault = function serveDefault(req, res, next) {
	...
	return statAndPipeFile(path, 'default', req, res);
};
...
ConventionalHandler.prototype.serveDefaultJson = function serveDefaultJson(req, res) {
	...
	return statAndPipeFile(path, 'default', req, res, {format: 'json'});
};
ConventionalHandler.prototype.serveTrackownership = function serveTrackownership(req, res) {
	res.header('Content-Type', 'application/json; charset=utf-8');
	return statAndPipeFile('/trackownership/user/', 'default', req, res, {format: 'json'});
};
...
ConventionalHandler.prototype.fileNamedAfterParameter = function fileNamedAfterParameter(matcher, req, res, next) {
	...
	if (fileName) {
		return statAndPipeFile(path, fileName, req, res);
	} else {
		return res.send(404);
	}
...
ConventionalHandler.prototype.usersUpdateIdSlug = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function findIdSlug(req) {
...
ConventionalHandler.prototype.tags = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function tags(req) {
	var matchingParameter, pageParam, page;

	matchingParameter = findMatchingParameter(function findTagParam(paramName) {
		return paramName.toLowerCase() === 'tags';
	}, req);

	pageParam = findMatchingParameter(function findPageParam(paramName) {
		return paramName.toLowerCase() === 'page';
	}, req);

	page = req.query[pageParam] || '1';

	return req.query[matchingParameter] + '-page-' + page;
});
...
// Finds a file named by the value of a query string parameter named 'emailAddress'
// and its value  E.g. 'emailAddress-a@b.com'
ConventionalHandler.prototype.findByEmailAddress = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function userId(req) {
	var email, matchingParameter;

	matchingParameter = findMatchingParameter(function findUserIdParam(paramName) {
		return paramName.toLowerCase() === 'emailaddress';
	}, req);

	email = decodeURIComponent(req.query[matchingParameter]);
	return {
		basePath: '/users/find/',
		fileName: 'emailAddress-' + email
	};
});

ConventionalHandler.prototype.emailAddress = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function userId(req) {
	var email, matchingParameter;
	...
	email = req.method = 'POST' ? req.body[matchingParameter] : decodeURIComponent(req.query[matchingParameter]);
	return email;
});
...
// Finds a file named by the value of query string parameters named 'artistId' and 'page'
ConventionalHandler.prototype.artistIdPaging = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function search(req) {
	var artistId, page, pageParam;

	artistId = findMatchingParameter(function findArtistIdParam(paramName) {
		return paramName.toLowerCase() === 'artistid';
	}, req);
	...
	return req.query[artistId] + '-page-' + page;
});
...
ConventionalHandler.prototype.serveTrackFile = function serveTrackFile(req, res) {
	...
	if (file.existsSync('./responses/' + parsedUrl.pathname + '/' + track + '.mp3')) {
		res.header('Content-Disposition', 'attachment; filename="' + track + '.mp3"');
		return statAndPipeFile(parsedUrl.pathname, track, req, res, {format: 'mp3'});
	} else {
		res.header('Content-Disposition', 'attachment; filename="default.mp3"');
		return statAndPipeFile(parsedUrl.pathname, 'default', req, res, {format: 'mp3'});
	}
};
...
ConventionalHandler.prototype.serveZipFile = function serveZipFile(req, res) {
	...
	if (file.existsSync('./responses/' + parsedUrl.pathname + '/' + id + '.zip')) {
		res.header('Content-Disposition', 'attachment; filename="' + id + '.zip"');
		return statAndPipeFile(parsedUrl.pathname, id, req, res, {format: 'zip'});
	} else {
		res.header('Content-Disposition', 'attachment; filename="default.zip"');
		return statAndPipeFile(parsedUrl.pathname, 'default', req, res, {format: 'zip'});
	}
};
...
ConventionalHandler.prototype.paypalComplete = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function paypalComplete(req) {
	var matchingParameter = findMatchingParameter(function findTokenCodeParam(paramName) {
		return paramName.toLowerCase() === 'token';
	}, req);

	return "token-" + req.body[matchingParameter];
});
