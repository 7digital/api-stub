...
function statAndPipeFile(filePath, fileName, res, opts) {
	...
	fullFilePath = path.join(__dirname, '../responses', filePath, fileName + '.' + opts.format);
...
ConventionalHandler.prototype.shopIdAndKey = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function findFileDetailsForMerchandising(req) {
    var reqPath = url.parse(req.url).pathname,
        ...
        shopId = req.query[shopIdParameter] === 'undefined' ? 34 : req.query[shopIdParameter];

	return {
		basePath: reqPath + '/' + shopId,
		fileName: key
	};
});

ConventionalHandler.prototype.country = ConventionalHandler.prototype.fileNamedAfterParameter.bind(ConventionalHandler, function tag(req) {
	var matchingParameter = findMatchingParameter(function findTagParam(paramName) {
...
// Attempts to serve an MP3 file with name corresponding to the trackid parameter on the request, in a folder
// relative to ./responses/, corresponding to the path of the request. If this file doesn't exist, it attempts the
// same for a file named 'default.mp3' in the same location, and if that doesn't exist, returns a 404.
...
// Attempts to serve a ZIP archive with name corresponding to the releaseid or purchaseid parameter on the request,
// in a folder relative to ./responses/, corresponding to the path of the request. If this file doesn't exist, it
// attempts the same for a file named 'default.mp3' in the same location, and if that doesn't exist, returns a 404.
